...
/** Category-wise subscription for one symbol. Returns null if unavailable. */
...
async function getIposEnriched() {
  ...
  const wanted = ipos.filter(
    (r) => r.symbol && r.start && r.start <= today && (!r.end || r.end >= recent)
  );
    ...
    Array.from({ length: Math.min(concurrency, queue.length) }, async () => {
      while (queue.length) {
        ...
        try {
          ...
          if (detail && detail.categories.length) out[item.symbol] = detail;
        } catch {
