...
async function getFundamentalsFor(pageUrl) {
  ...
  const tables = tableRows(await res.text());
  ...
  if (!financials.length && !Object.keys(kpis).length && !peers.length) return null;
...
module.exports = {
  ...
  getFundamentalsFor, getFundamentals, parseFigure,
